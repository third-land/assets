...
async function resolveNotFound(request: Request): Promise<Response> {
  ...
  if (fallback === "true") {
    const baseURI =
      "https://raw.githubusercontent.com/SmolDapp/tokenAssets/main/_config/nodeAPI/public";
    const result = await fetch(`${baseURI}/not-found.png`);
    return new Response(result.body, {
      headers: {
        "Content-Type": "image/png",
        "Cache-Control":
          "public, max-age=60, stale-while-revalidate=3600, must-revalidate",
      },
    });
  }
...
export async function GET(
  ...
  const finalURI =
    `${baseURI}/tokens/${chainIDStr}/${tokenAddress}/${fileName}`.toLowerCase();
